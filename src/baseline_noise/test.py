...
def main(fold_list, fold_root, max_len, feature, seed):
    ...
    for fold in fold_list:
        ...
        np.save(os.path.join(root,'pred_test_snr_{}_snr{}.npy'.format(args.noise_type,args.snr)),res_test['pred_all'])
    ...
    with open('./test_all_seed_{}_{}.txt'.format(seed,args.dataset_type),'a') as f:
    ...
    with open('./test_all_seed_{}_{}_global.txt'.format(seed,args.dataset_type),'a') as f:
        f.write('feature:{} max_len:{} snr:{} noise_type:{} do_se:{}\n'.format(feature,max_len,args.snr,args.noise_type,args.do_se))
        f.write('fold_list:{}  Class Names: {}\n'.format(fold_list,target_names))
