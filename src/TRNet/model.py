...
class Frontend(nn.Module):
    def __init__(self, feature_size=80):
        ...
        self.activate = nn.Hardtanh(0,1)
        #self.fc2 = nn.Linear(feature_size,1)

    def forward(self, x_n, x_e):
        corr = self.cos(x_n, x_e)
        S = self.activate(self.fc(corr)).unsqueeze(-1)
        #bias = self.fc2(corr).unsqueeze(-1)
        S_new = S*x_n + (1-S)*x_e
        return S_new, S.squeeze(1)

class Backend(nn.Module):
    def __init__(self, h_dims=256):
        super(Backend, self).__init__()
        self.f = nn.Linear(1, h_dims)
        self.h = nn.Linear(1, h_dims)
    def forward(self, x, s):
        return self.f(s)*x + self.h(s)
...
class Mymodel(nn.Module):
    def __init__(self, feature_size=80, h_dims=256, emotion_cls=4, backbone_type='CNN6'):
        ...
        self.front_model = Frontend(feature_size=feature_size)
        self.back_model = Backend(h_dims=h_dims)
    ...
    def forward(self, spec_n, spec_e):
        spec_new, S_score = self.front_model(spec_n,spec_e)
        x = self.transform(spec_new)
        x = self.backbone(x)
        embedding, att, _ = self.att_block(x)
        proj_ebd = self.back_model(embedding, S_score)
        pred, _ = self.EmotionClassifier(proj_ebd)
        return pred, embedding, spec_new, S_score, proj_ebd